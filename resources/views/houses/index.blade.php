...
<div id="app-content">
    ...
    @endif
        ...
        <div class="card-footer d-flex gap-2 flex-wrap">
            ...
            <a href="{{ route('houses.3d_view', $house->id) }}" class="btn btn-info mt-2">
                View in 3D
            </a>
                ...
                <form action="{{ route('houses.update', ['house' => $house->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        @foreach ($imageTypes as $type)
                            ...
                            @if ($image)
                                <div class="mb-2">
                                    <img src="{{ $image->img_path }}" alt="{{ $type }} image" class="img-thumbnail" style="height: 120px;">
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            @if ($house->model_path)
                                <div class="mb-2">
                                    <p>Current model: <a href="{{ Storage::url($house->model_path) }}" target="_blank">View</a></p>
                                </div>
                            @endif
                            <input type="file" class="form-control" name="model" accept=".gltf,.glb">
                            <small class="text-muted">Leave blank to keep current model</small>
                        </div>
                        <div class="mb-3">
...
<div class="modal fade" id="viewHouseModal{{ $house->id }}" tabindex="-1" aria-labelledby="viewHouseModalLabel{{ $house->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <p class="mb-3"><strong>Name:</strong>{{ $house->name }}</p>
                    <p class="mb-3"> <strong>City:</strong>{{ $house->city }}</p>
                    <p class="mb-3"><strong>Price:</strong>KES {{ number_format($house->price, 2) }}</p>
                    <p class="mb-3"><strong>Location:</strong>{{ $house->location }}</p>
                    <p class="mb-3"><strong>Bedrooms:</strong>{{ $house->bedrooms }}</p>
                    <p class="mb-3"><strong>Description:</strong>{{ $house->description }}</p>
                    <p class="mb-3"><strong>Status:</strong>{{ ucfirst($house->status) }}</p>
                    @auth
                    @if(auth()->user()->role === 'admin')
                    <p class="mb-3"> <strong>Created At:</strong>{{ $house->created_at->format('Y-m-d H:i') }}</p>
                    <p class="mb-3"><strong>Updated At:</strong>{{ $house->updated_at->format('Y-m-d H:i') }}</p>
                    @endif
                    @endauth
            </div>
            <div class="modal-footer">
